...
<div class="container_div">
    <div class="header">Статистика по Спецализации</div>
    <div class="form-div">
        <form action="/indicators-specialization" method="post" enctype="multipart/form-data">
            @csrf
    ...
    <div class="table-div">
        ...
        <table class="table table-value table1 spec_indicator">
            <tbody>
                ...
                <tr>
                    ...
                    <td><div class="value default">  0 %</div></td>
                    @elseif($value['sum(first_therapy)'] == 0 || $value['sum(first_consulting)'] == 0 )
                    <td><div class="value red">  0 %</div></td>
                    @else
                    <?php
                    $number = number_format($value['sum(first_therapy)'] / $value['sum(first_consulting)'] * 100, 0, ",", " ");

                    if (isset($reference_array[$value['id']])) {
                        $id = $value['id'];
                    ...
                    <td><div class="value default">  0 RUR</div></td>
                    ...
                    @else
                    <?php
                    $number = number_format($value['sum(spent_time)'] / $value['sum(schedule_time)'] * 100, 0, ",", " ");
        ...
        <table class="table conversion_admin table-head table1 doctor_stat monthly_stat">
            <thead>
                <tr>
                    ...
                    <th scope="row" class="color_th">{{$value}}</th>
                    @endforeach
                </tr>
            </thead>
        </table>
        <table class="table table-value table1 admin_indicator admin_indicator_monthly">
            <tbody>
                <?php
                foreach ($final as $spec_name => $value_spec) {

                    $i = 1;
                    ?>
                    <tr>
                        <?php
                        foreach ($value_spec as $month => $value) {

                            if ($i == 1) {
                                echo '<td class="color_th">' . $spec_name . '</td>';
                            }
                        ...
                        foreach ($value_spec as $month => $value) {
                            ...
                            if (!isset($value['spent_time']) || !isset($value['schedule_time'])) {
                                ...
                                if ($value['spent_time'] == 0 || $value['schedule_time'] == 0) {
                                    ...
                                    $number = round($value['spent_time'] / $value['schedule_time'] *100);
                                    $number = number_format($number, 0, ",", " ");
                                    if (isset($reference_array[$value['specializ_id']])) {
                                        $id = $value['specializ_id'];
                                        if ($number >= $reference_array[$id]['conversion_good'] && $reference_array[$id]['conversion_good'] != null) {
